%% Example: binary classification using generalized linear model
...
[xVal,componentLabels] = generateDataFromGMM(NVal,gmmParameters); % used to have a 3rd argument of 0.
n = size(xVal,1);
...
for i=1:length(NTrain) % Conduct experiments with different amounts of training samples
    ...
    [xTrain{i},componentLabels] = generateDataFromGMM(NTrain(i),gmmParameters); % used to have a 3rd argument of 0.
...
NTrain2 = [NTrain; NTrain];
error_L = [error_L_GD error_L_SGD]';
error_quad = [error_quad_GD error_quad_SGD]';
TableFill = [NTrain2 error_L error_quad];
...
fprintf('\t  %i\t\t\t\t\t %.2f%%\t\t\t\t\t\t\t%.2f%%\n',TableFill'); 
% First three are using gradient descent, the second three rows are
