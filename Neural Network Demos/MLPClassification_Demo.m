%% MLPClassification_Demo.m
% NOT YET TESTED... This was grabbed from Assignment3.m script and hastily
% thrown in here.
...
% Creates a gaussian mixture model (GMM) from which classifications can be made
clear all; close all;

% Setup: create multiples sets of data samples based on data distribution 
...
% load in data 
load("MLPDemoDataset.mat");
N_vector = N;

x = dataset(7).x; % check orientation of this... may not jive.
...
%% Sample MLP construct and train

dsn = 4; % choose dataset number
x = dataset(dsn).x; % choose dataset dsn (xxxx samples)
